...
func parseInt(s string, base int) (Value, error) {
	...
	for ; i < len(s); i++ {
		if n >= cutoff {
			// n*base overflows
			return parseLargeInt(float32(n), s[i:], base, sign)
		}
		v := digitVal(s[i])
		if v >= base {
			break
		...
		if n1 < n || n1 > maxVal {
			// n+v overflows
			return parseLargeInt(float32(n)+float32(v), s[i+1:], base, sign)
		}
		n = n1
	}
...
func parseLargeInt(n float32, s string, base int, sign bool) (Value, error) {
	i := 0
	b := float32(base)
	for ; i < len(s); i++ {
		v := digitVal(s[i])
