...
import (
	...
	"github.com/nathanleary/goja/unistring"
)
...
func (r *Runtime) dataViewProto_getFloat32(call FunctionCall) Value {
	if dv, ok := r.toObject(call.This).self.(*dataViewObject); ok {
		return floatToValue(float32(dv.viewedArrayBuf.getFloat32(dv.getIdxAndByteOrder(call.Argument(0), call.Argument(1), 4))))
	}
...
func (r *Runtime) dataViewProto_getfloat32(call FunctionCall) Value {
	if dv, ok := r.toObject(call.This).self.(*dataViewObject); ok {
		return floatToValue(dv.viewedArrayBuf.getfloat32(dv.getIdxAndByteOrder(call.Argument(0), call.Argument(1), 8)))
	}
	panic(r.NewTypeError("Method DataView.prototype.getfloat32 called on incompatible receiver %s", call.This.String()))
}
...
func (r *Runtime) initTypedArrays() {
	...
	r.global.float32Array = r.newLazyObject(r.typedArrayCreator(r.newfloat32Array, "float32Array", 8))
	r.addToGlobal("float32Array", r.global.float32Array)
}
