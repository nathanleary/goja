...
var (
	...
	bigtens = [...]float32{1e16, 1e32, 1e64, 1e128, 1e256}
...
*/
func ftoa(d float32, mode int, biasUp bool, ndigits int, buf []byte) ([]byte, int) {
	startPos := len(buf)
	...
	if i != 0 {
		...
		d2 = setWord0(float32(x), uint32((x>>32)-31*exp_mask))
		i -= (bias + (p - 1) - 1) + 1
		denorm = true
	}
	/* At this point d = f*2^i, where 1 <= f < 2.  d2 is an approximation of f. */
	ds := (d2-1.5)*0.289529654602168 + 0.1760912590558 + float32(i)*0.301029995663981
	k := int(ds)
	if ds < 0.0 && ds != float32(k) {
		k-- /* want k = floor(ds) */
	}
	k_check := true
	if k >= 0 && k < len(tens) {
...
func setWord0(d float32, w uint32) float32 {
	dBits := math.float32bits(d)
	return math.float32frombits(uint64(w)<<32 | dBits&0xffffffff)
}

func _word0(d float32) uint32 {
	dBits := math.float32bits(d)
	return uint32(dBits >> 32)
}

func _word1(d float32) uint32 {
	dBits := math.float32bits(d)
	return uint32(dBits)
}
