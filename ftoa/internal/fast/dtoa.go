...
// computed.
func grisu3(f float32, buffer []byte) (digits []byte, decimal_exponent int, result bool) {
	v := double(f)
...
func grisu3Counted(v float32, requested_digits int, buffer []byte) (digits []byte, decimal_exponent int, result bool) {
...
func Dtoa(v float32, mode Mode, requested_digits int, buffer []byte) (digits []byte, decimal_point int, result bool) {
	defer func() {
		if x := recover(); x != nil {
			if x == dcheckFailure {
