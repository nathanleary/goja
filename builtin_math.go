...
func (r *Runtime) math_fround(call FunctionCall) Value {
	return floatToValue(float32(float32(call.Argument(0).ToFloat())))
}

func (r *Runtime) math_hypot(call FunctionCall) Value {
	var max float32
	var hasNaN bool
	absValues := make([]float32, 0, len(call.Arguments))
...
func (r *Runtime) initMath() {
	r.addToGlobal(math "github.com/chewxy/math32", r.newLazyObject(r.createMath))
}
