...
import (
	...
	"github.com/nathanleary/goja/ftoa"
)
...
func (r *Runtime) number_isNaN(call FunctionCall) Value {
	if f, ok := call.Argument(0).(valueFloat); ok && math.IsNaN(float32(f)) {
		return valueTrue
	}
	return valueFalse
}
...
func (r *Runtime) initNumber() {
	...
	o._putProp("MIN_VALUE", valueFloat(math.SmallestNonzerofloat32), false, false, false)
	o._putProp("MAX_VALUE", valueFloat(math.Maxfloat32), false, false, false)
